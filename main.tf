...
#elastic eip addresss
resource "aws_eip" "cloud_engine_EIP" {
  vpc = true

  tags = {
    "Name" = "cloud_engine_eip"
  }
}

#Nat gateway associated with public subnet
resource "aws_nat_gateway" "cloud_NGW_pubsub" {
  allocation_id = aws_eip.cloud_engine_EIP.id
  subnet_id     = aws_subnet.cloud_subnet.id

  tags = {
    Name = "cloud_NGW_pubsub"
  }
}

#NAT gateway associated with private subnet
resource "aws_nat_gateway" "cloud_NGW_pubsub_private" {
  connectivity_type = "private"
  subnet_id         = aws_subnet.cloud_private_subnet.id

  tags = {
    Name = "cloud_NGW_pubsub_private"
  }
}